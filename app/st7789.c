...
int back_color = 1; // Blue

void draw_point(uint16_t x, uint16_t y, uint16_t color) {
    volatile uint16_t *p = (volatile uint16_t *)(0x20000000 + (y << 8) + x);
    *p = color;
}
...
void LCD_prints_color(const char *str, uint16_t color) {
    while (*str) {
        if (*str == '\n') {
            ...
            draw_char(st7789_col << 4, st7789_row << 4, *str, color, 1);
            update_pos();
        }
        str++;
    }
